...
<html class="no-js">
    ...
    <head>
        <meta charset="utf-8"/>
        <title>School portal :: Account Module :: Payment Log</title>
	...
	function hideLoader(){
      ...
      $('#calp').submit(function(e) {
      showLoader();
      var t = $("#term").val();
      var term = t.replace(" ", "%20");
      var s = $("#sess").val();
      var sess = s.replace(" ", "%20");
	   var c = $("#class").val();
      var clas = c.replace(" ", "%20");
      $("#content").load("../bin_bursar/calpayment.php?t="+term+"&s="+ sess +"&class="+clas, hideLoader());
                ...
                <div id="page-content-sb" class="cf">        	
                    ...
                    <div class="entry-content cf">
                        ...
                        <div id="mai" style="margin-bottom: 10px;">
                            <form action="" method="post" id="calp">
                                    <select name="term" id="term">
                                        <option value="">Select Term</option>
                                        <?php 
                                        $log->set_sqlstr("SELECT DISTINCT Term FROM paymentlog WHERE Class ='".$_REQUEST['class']."'"); 
                                        $log->querydata();                        
                                    ...
                                    <select name="sess" id="sess">
                                        ...
                                        <?php 
                                        $log->set_sqlstr("SELECT DISTINCT Session FROM paymentlog WHERE Class ='".$_REQUEST['class']."'"); 
                                        $log->querydata();                        
                                        for($i=0; $i < $log->no_rec; $i++){
                                           echo "<option value=\"".$log->data[0]."\">".$log->data[0]."</option>"; 
                                           $log->fetchdata();
									...
									<input type="hidden" value="<?php echo $_REQUEST['class']; ?>" id="class"/>
