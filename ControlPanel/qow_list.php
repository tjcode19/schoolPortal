...
<html class="no-js">
    ...
    <head>
        <meta charset="utf-8"/>
        <title>Demo School :: Control Panel :: Staff Log</title>
	...
	$("#adds").click(function(){
		...
		$("#content").load("../bin_cpanel/qow_list.php?page=" + this.id, hideLoader);
	...
	showLoader();
	$("#content").load("../bin_cpanel/qow_list.php?page=1", hideLoader);
});
...
$per_page = 20;  
$log->set_sqlstr("select * from auth_tab WHERE Priority='admin' ");
$log->querydata(); 
$count = $log->no_rec;
	...
	bring(selecter);
     //callback handler for select change of option for state
       $('#m').submit(function(e) {
        ...
        $.ajax(
        {
            url : "../bin_cpanel/edit_question.php",
            type: "POST",
            data : postData,
            //dataType: 'json',
            dataType: 'html',
            success:function(d)
            {
                //data: return data from server
                    ...
                    $("#content").load("../bin_cpanel/qow_list.php?page=1&sid="+ d, hideLoader()); 
                    $("#sent-form-msg").html("Question Updated Successfully").show(hideLoader()); 
            },
