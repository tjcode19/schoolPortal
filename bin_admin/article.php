...
if(isset($_REQUEST['act']) && ($_REQUEST['act'] == "show")){
 $log->set_sqlstr("UPDATE publications SET Status=1, user_id='".$_SESSION['user_id']."' WHERE id = '" . $_REQUEST['n'] . "'");
        $log->ex_scalar();    
}
$per_page = 20;
$page = $_REQUEST['page'];
...
$log->set_sqlstr("select * from publications WHERE Type=2 order by id DESC limit $start,20");
$log->querydata(); 
//$rsd = mysql_query($sql);
if($log->no_rec == 0){    $m = "
