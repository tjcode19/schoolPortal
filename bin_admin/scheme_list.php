...
function getStatus($st){
			if($st == 0){$status = "Pending";}
			elseif($st == 1){$status = "Done";}
			return $status;
		}
?>
...
{?>
     <div class="row">
        ...
        <div class="middle"><?php echo getStatus($log->data['Status']); ?></div>
        <div class="right">
            <?php echo (($log->data['Status'] == 0)?"<a href=\"../bin_admin/scheme_list.php?page=1&st=done&n=". $log->data['ID'] ."&c=".$_REQUEST['c']."&s=".$_REQUEST['s']."&t=".$_REQUEST['t']."\" class=\"link-button\">Set as Done</a>" :
