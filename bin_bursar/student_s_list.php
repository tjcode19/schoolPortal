...
$se = isset($_REQUEST['sid']) ? $_REQUEST['sid'] : "";
        $str_sql = ($se!="") ? "SELECT * FROM status INNER JOIN student_data ON status.Username=student_data.Username".
                                " WHERE student_data.Class LIKE '%".$se."%'" :
                                "select * from status INNER JOIN student_data ON status.Username=student_data.Username order by status.ID DESC limit $start,20";
...
{?>
    ...
    <div class="row">
        ...
        <div class="right">
          <a href="../bin_admin/status_v.php?n=<?php echo $log->data['Username']; ?>">
                        <img class="alignleft" alt="View" title="View Details" src="../img/mono-icons/paper32.png" />
                    </a>
                    <a href="../bin_admin/status_u.php?n=<?php echo $log->data['Username']; ?>">
