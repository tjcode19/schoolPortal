...
$log->set_sqlstr("select * from publications WHERE Type='Jokes' order by ID ASC limit $start,20");
$log->querydata(); 
//$rsd = mysql_query($sql);
if($log->no_rec == 0){    $m = "
...
{?>
     <div class="row" id="jokes">
        ...
        <div class="right">
            <a href="../bin/article_view.php?page=1&n=<?php echo $log->data['ID']; ?>" class="link-button" title="Read Joke">R</a>
            <a href="../bin_cpanel/jokes.php?page=1&act=del&n=<?php echo $log->data['ID']; ?>" class="link-button" title="Delete Joke">D</a>
            <?php 
                if($log->data['Status'] == 1){
            ?>
            <a href="../bin_cpanel/jokes.php?page=1&act=hide&n=<?php echo $log->data['ID']; ?>" class="link-button red" title="Hide Joke">H</a>
            <?php 
                }else{
            ?>
             <a href="../bin_cpanel/jokes.php?page=1&act=show&n=<?php echo $log->data['ID']; ?>" class="link-button green" title="Show Joke">S</a>
            <?php } ?>
...
$(document).ready(function(){
	...
	bring(selecter);
         ...
         $('#container #jokes .right a:last-child').click(function(e){
             //showLoader();
             var link = this.href;
               $("#jokes").load(link);             
               e.preventDefault();
	});
         $('#container #jokes .right a:first-child').click(function(e){
             //showLoader();
             var link = this.href;
             $("#show").load(link);
             $('#mwindow').show();              
        ...
        $('#container #jokes .right a:nth-child(2)').click(function(e){
             //showLoader();
