...
$log->set_sqlstr("select * from publications WHERE Type='News' order by ID DESC limit $start,20");
$log->querydata(); 
//$rsd = mysql_query($sql);
if($log->no_rec == 0){    $m = "
...
{?>
     <div class="row" id="news">
        ...
        <div class="right">
            ...
            <a href="../bin_cpanel/news.php?page=1&act=del&n=<?php echo $log->data['ID']; ?>" class="link-button" title="Delete News">D</a>
            <?php 
                if($log->data['Status'] == 1){
            ?>
            <a href="../bin_cpanel/news.php?page=1&act=hide&n=<?php echo $log->data['ID']; ?>" class="link-button red" title="Hide News">H</a>
            <?php 
                }else{
            ?>
             <a href="../bin_cpanel/news.php?page=1&act=show&n=<?php echo $log->data['ID']; ?>" class="link-button green" title="Show News">S</a>
            <?php } ?>
        </div>
    </div>
