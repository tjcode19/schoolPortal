...
$se = isset($_REQUEST['sid']) ? htmlspecialchars($_REQUEST['sid'],ENT_QUOTES) : "";
        $str_sql = ($se!="") ? "SELECT * FROM authentication INNER JOIN staff_data ON authentication.id=staff_data.id".
                                ...
                                "%' OR staff_data.last_name LIKE '%".$se."%' OR authentication.username LIKE '%".$se."%'" :
                                "select * from authentication INNER JOIN staff_data ON authentication.id=staff_data.id WHERE authentication.priority=2 ".
								"order by authentication.id DESC limit $start,20";
...
{?>
           ...
           <tr align="center">
                ...
                <td><?php echo $log->data['first_name'].", ".$log->data['last_name']; ?></td>
