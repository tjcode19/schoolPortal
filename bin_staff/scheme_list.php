...
if(isset($_REQUEST['st']) && ($_REQUEST['st'] == "pd")){
    $log->set_sqlstr("UPDATE schemeofwork SET Status=0 WHERE ID = '" . $_REQUEST['n'] . "'");
        $log->ex_scalar();    
}
elseif(isset($_REQUEST['st']) && ($_REQUEST['st'] == "done")){
    $log->set_sqlstr("UPDATE schemeofwork SET Status=1 WHERE ID = '" . $_REQUEST['n'] . "'");
        $log->ex_scalar();    
}
...
{?>
     <div class="row">
        ...
        <div class="right">
            <?php echo (($log->data['Status'] == 0)?"<a href=\"../bin_staff/scheme_list.php?page=1&st=done&n=". $log->data['ID'] ."&c=".$_REQUEST['c']."&s=".$_REQUEST['s']."&t=".$_REQUEST['t']."\" class=\"link-button\">Set as Done</a>" :
                    "<a href=\"../bin_staff/scheme_list.php?page=1&st=pd&n=". $log->data['ID'] ."&c=".$_REQUEST['c']."&s=".$_REQUEST['s']."&t=".$_REQUEST['t']."\" class=\"link-button\">Set as Pending</a>"); ?></div>
    </div>
